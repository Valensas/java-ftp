...
class FtpApplicationTests {
	...
	fun `Test sftp connection`() {
		assertDoesNotThrow {
			...
			client.connect(ConnectionModel(ConnectionType.FTPS, server.getHost(), server.getPort(), "username", "password", Fake.publicKey(), Fake.privateKey(), null))
	...
	fun `Test if fail sftp connection can be handled`() {
		assertThrows<Exception> {
			...
			server.start("username", "password")
			val client = ftpClientFactory.createFtpClient(ConnectionType.SFTP) as SFTPClient
			client.connect(ConnectionModel(ConnectionType.FTPS, server.getHost(), server.getPort(), "wrongusername", "password", Fake.publicKey(), Fake.privateKey(), null))
			assertTrue(client.isConnected)
			client.disconnect()
			server.stop()
		}
